...
import { extraerNombre, extraerProducto, clasificarConScoring, clasificarConIA } from '../webhook/classifier.js'
...
async function escribirEnSheets(data) {
  ...
  try {
    const params = new URLSearchParams({
      accion: data.accion || 'nuevo', telefono: data.telefono || '',
      msgInicial: (data.msgInicial || '').slice(0, 200),
      mensajes: (data.mensajes || '').slice(0, 500),
      nombre: data.nombre || '', producto: data.producto || '',
      perfil: data.perfil || '', prioridad: data.prioridad || '',
      estado: data.estado || '', vendedor: data.vendedor || '', campana: data.campana || ''
    })
    await fetch(`${url}?${params}`, { method: 'GET', redirect: 'follow', signal: AbortSignal.timeout(8000) })
...
async function clasificar(texto) {
  ...
  if (scoring.confianza === 'baja' && process.env.GROQ_API_KEY) {
    try { clasif = { ...scoring, ...await clasificarConIA(texto) } } catch {}
  }
...
async function guardarMsg(prisma, leadId, convId, texto) {
  try { await prisma.message.create({ data: { leadId, conversationId: convId || null, origen: 'BOT', texto } }) } catch {}
}
async function enviarLead(inst, tel, msg) {
  try { await enviarTexto(inst, tel, msg) } catch(e) { console.error(`[Followup] lead ${tel}:`, e.message) }
}
async function enviarVendedor(inst, num, msg) {
  try { await enviarTexto(inst, num, msg) } catch(e) { console.error('[Followup] vendor:', e.message) }
}
...
export async function ejecutarFollowup(prisma) {
  ...
  for (const conv of convsActivas) {
    try {
      ...
      // Sin más pasos → notificar vendedor → NOTIFIED
      if (conv.vendorNotificationCount === 0) {
        for (const n of notifys) {
          if (conv.vendor?.whatsappNumber)
            await enviarVendedor(instancia, conv.vendor.whatsappNumber, interp(n.mensaje, vars))
        }
        if (notifys.length === 0 && conv.vendor?.whatsappNumber) {
          const emoji = clasif.prioridad === 'URGENTE' ? '🔴' : clasif.prioridad === 'ALTA' ? '🟠' : '🟡'
          const msgV = `${emoji} NUEVO LEAD — ${clasif.prioridad}\n\n📱 wa.me/${conv.lead.telefono}\n👤 ${clasif.nombre || 'Sin nombre'}\n📦 ${clasif.producto || 'Sin producto'}\n🎯 ${clasif.tipoPreciso || ''}\n📚 ${conv.campaign?.nombre || 'orgánico'}${historial ? `\n\n💬 Dijo:\n${historial}` : ''}\n\n⚡ ${clasif.prioridad === 'URGENTE' ? '¡Llama AHORA!' : 'Llama hoy'}`
          await enviarVendedor(instancia, conv.vendor.whatsappNumber, msgV)
        }
        await escribirEnSheets({
          ...
          estado: 'pendiente llamar', vendedor: conv.vendor?.nombre || '',
    ...
    } catch (err) { console.error(`[Followup] Error conv ${conv.id}:`, err.message) }
  }

  // ── FASE 2: FOLLOWUP steps ─────────────────────────────────
  const convsF = await prisma.conversation.findMany({
    where: { state: { in: ['ACTIVE', 'REACTIVATED'] }, campaignId: { not: null } },
    include: { lead: true, campaign: { include: { steps: { orderBy: { orden: 'asc' } } } }, vendor: true }
  })
  ...
  for (const conv of convsF) {
    ...
    } catch (err) { console.error(`[Followup] Error followup ${conv.id}:`, err.message) }
  }

  // ── FASE 3: Reactivaciones ─────────────────────────────────
  ...
  for (const conv of convsR) {
    try {
      ...
      if (minutos >= 2880) {
        await prisma.conversation.update({ where: { id: conv.id }, data: { state: 'CLOSED' } })
      ...
      if (minutos >= 1440 && reactCount === 0) {
        ...
        if (conv.vendor?.whatsappNumber)
          await enviarVendedor(instancia, conv.vendor.whatsappNumber, `🔔 Lead 30min esperando\n📱 wa.me/${conv.lead.telefono}`)
        await prisma.conversation.update({
          where: { id: conv.id },
      ...
      await prisma.conversation.update({
        where: { id: conv.id },
        data: { reactivationCount: reactCount + 1, lastReactivationAt: new Date(), lastBotMessageAt: new Date() }
      })
      procesados++
      console.log(`[Followup] Reactivación #${reactCount + 1}: ${conv.lead.telefono}`)
    } catch (err) { console.error(`[Followup] Error reactivando ${conv.id}:`, err.message) }
  }
